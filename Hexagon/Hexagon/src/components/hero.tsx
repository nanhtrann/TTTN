...
const defaultServices = ["Cung cấp thiết bị CNTT", "Dịch vụ CNTT", "Giải pháp công nghệ", "Thi công & Lắp đặt"];
...
export const Hero = ({
  ...
  const primaryText = primaryButtonText || t.btn1;
  const secondaryText = secondaryButtonText || t.btn2;

  const serviceList: string[] = (services && services.length)
    ? services.map((s: any) => (typeof s === 'string' ? s : s.text || s.label || s))
    : ((t as any).services || defaultServices);
  ...
  useEffect(() => {
    const handleType = () => {
      const i = loopNum % serviceList.length;
      const fullText = serviceList[i];
  ...
  return (
    ...
    >
      <div className="max-w-xl z-10">
        ...
        <p className="text-base opacity-90 mb-8 leading-relaxed">{descText}</p>
      ...
      <a href="#content" className="absolute bottom-8 left-1/2 -translate-x-1/2 text-white/70 text-sm flex flex-col items-center gap-1 hover:text-white transition-colors">
        <span>{scrollHint || 'Cuộn xuống để khám phá'}</span>
